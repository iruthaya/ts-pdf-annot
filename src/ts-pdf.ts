...
export class TsPdfViewer {
  ...
  constructor(options: TsPdfViewerOptions) {
    ...
    this._mainContainer.addEventListener("click", (e) => {
      ...
      // const parentEle = dialog.querySelector(".text-cancel");
      // const childEle = parentEle?.firstElementChild;
      var target = (e.target as HTMLElement);
      if(target.closest('.text-cancel') || target.classList.contains('.text-cancel')){
  ...
  protected onTextSelectionChange = () => {
    ...
    if ((<any>this._shadowRoot).getSelection) {
      selection = (<any>this._shadowRoot).getSelection();
  ...
  private setMode(mode?: ViewerMode) {
    mode = mode || this._modeService.enabledModes[2] || "annotation"; // 'text' is the default mode
  ...
  private onAnnotationChange = (e: AnnotEvent) => {
    ...
    switch(e.detail.type) {
      ...
      case "select":      
        ...
        const sel_annotation = annotations[0];
        if (sel_annotation) {
          (<HTMLParagraphElement>this._shadowRoot.querySelector("#focused-annotation-author"))
            .textContent = sel_annotation.author || "";
          (<HTMLParagraphElement>this._shadowRoot.querySelector("#focused-annotation-date"))
            .textContent = new Date(sel_annotation.dateModified || sel_annotation.dateCreated).toDateString();
          (<HTMLParagraphElement>this._shadowRoot.querySelector("#focused-annotation-text"))
            .textContent = sel_annotation.textContent || "";
        } else {
          (<HTMLParagraphElement>this._shadowRoot.querySelector("#focused-annotation-author"))
            .textContent = "";
          (<HTMLParagraphElement>this._shadowRoot.querySelector("#focused-annotation-date"))
    ...
    if (this._annotChangeCallback) {
      ...
      this.setAnnotationMode("preselect");
    }
  };
